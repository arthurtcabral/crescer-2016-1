...
$(function() {
    ...
    $('button[name=btnCarregaCavaleiros]').click(function() {
      // significa que montou todos objetos que representam o html
      ...
      var goldSaintsAdaptados = goldSaints;
      ...
      for (var i = 0; i < goldSaintsAdaptados.length; i++) {
        goldSaintsAdaptados[i].alturaCm = cmParaMetros(goldSaintsAdaptados[i].alturaCm) + " m";
      }

      for (var i = 0; i < goldSaintsAdaptados.length; i++) {
        if(apenasComPesoDefinido(goldSaintsAdaptados[i])){
        goldSaintsAdaptados[i].pesoLb = Math.round(lbParaKilos(goldSaintsAdaptados[i].pesoLb)*100) / 100 + " kg";
        }
      }

      for (var i = 0; i < goldSaintsAdaptados.length; i++) {
        goldSaintsAdaptados[i].dataNascimento = converterData(goldSaintsAdaptados[i].dataNascimento);
      }

      $('#cavaleiros img').click(function(i) {
        var self = $(this);
        var nome = self.attr('alt');
        var altura = goldSaintsAdaptados.filter(function(elem) {
          return elem.id === parseInt(self.attr('id'));
        })[0].alturaCm;
        var $headers = $('#headers');
        $headers.append($('<h1>').text( nome ));
        $headers.append($('<h1>').text( altura ));
      });
      ...
      $("#cavaleiros img").hover(
          ...
          $('#div-mostra').show();
          var self = $(this);
          for (var campo in goldSaintsAdaptados[self[0].id-1]) {
            if (goldSaintsAdaptados[self[0].id-1].hasOwnProperty(campo)) {
              if(campo !== 'imagens'){
              $('#div-mostra').append(campo + ":" + goldSaintsAdaptados[self[0].id-1][campo] + " ").append('<br>');
              }
            }
          }
          $('#div-mostra').css('background-color', 'lightgrey');
          $('#div-mostra').css('border', 'solid 1px black');
  			},
  			function(){
